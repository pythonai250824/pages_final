...
3. Create Postgresql database for the project -- should be saved as queries : create table queries, insert queries  
   ...
   also upload to Github a few demos csv files for train  
   create ml_user table : id (autoincrement), email, pwd, tokens  
4. create endpoint -- "user/create" which gets user email + pwd and creates a new user with 15 tokens   
5. create endpoint -- "user/login" which gets user email + pwd and validate -- return "OK" if correct or "FAIL" if not  
6. create streamlit page for creation of new user, calls REST API  
7. create streamlit page for login of a user, calls REST API    
